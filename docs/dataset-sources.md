...
Sync hashes Lance datasets with up to **8 workers** in parallel, then compares each hash with `datasetHash` in the remote dataset's `quarry-storage.json`. Set `--hash-workers N` to change concurrency, or `--hash-workers 1` for serial hashing. Matching datasets print `Skip [UNCHANGED]` and bypass the upload helper entirely. New datasets, different hashes, and missing or invalid remote hashes trigger uploads. Only the small remote descriptors are downloaded, using the repository commit captured at the start of sync. Remote lookup failures stop sync. Single-file datasets continue through the SDK's normal upload path. The summary reports uploaded and skipped counts; skipped datasets still participate in source recording and are kept during pruning.
...
`--dry-run` performs the same hash comparison without writing local hashes, labels matching datasets `Skip [UNCHANGED]`, and labels upload candidates `[NEW]` when their destination path is absent from HF or `[EXISTING]` when it is already present. It prints remote presence totals and upload/skip counts. This checks the remote repository independently of the source catalog and works with or without `--prune`. It also prints any proposed deletions and newly discovered source names without uploading, deleting, editing dataset or catalog JSON, or prompting. Remote descriptors may be downloaded to the SDK cache. `--catalog /path/to/sources.json` overrides the default checked-in file, whose location is anchored to this extension rather than the current working directory.
